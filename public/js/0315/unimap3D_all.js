...
var viewer = new Cesium.Viewer('cesiumContainer', {
    ...
    imageryProvider: undefined,//图像图层提供者，仅baseLayerPicker设为false有意义
...
binglayer = imageryLayers.get(0);
binglayer.name = "卫星+地形";
osmLayer = new Cesium.ImageryLayer(Cesium.createOpenStreetMapImageryProvider());
osmLayer.name = "地图";
...
function changeMap() {
    ...
    if (selectIndex == 1) {
        ...
        imageryLayers.remove(binglayer, false);
        imageryLayers.add(osmLayer);
        realHeight = false;
    } else {
        viewer.terrainProvider = cesiumTerrainProviderMeshes;
        imageryLayers.remove(osmLayer, false);
        imageryLayers.add(binglayer);
        realHeight = true;
    }
    reloadData();
}
...
function initProfile() {
    ...
    chartArray.forEach(function (chart) {
        ...
        d3.selectAll('.profile')
            .style({ opacity: '0.2', filter: "alpha(opacity=30)", "-moz-opacity": "0.3", "-khtml-opacity": "0.3" })
            .on('mouseover', function (d) {
                d3.selectAll('.profile').style({ opacity: '0.03', filter: "alpha(opacity=3)", "-moz-opacity": "0.03", "-khtml-opacity": "0.03" });
                d3.select(this).style({ opacity: '1.0', filter: "alpha(opacity=100)", "-moz-opacity": "1.0", "-khtml-opacity": "1.0" });
            })
            .on('mouseout', function (d) {
                d3.selectAll('.profile').style({ opacity: '0.3', filter: "alpha(opacity=30)", "-moz-opacity": "0.3", "-khtml-opacity": "0.3" })
            })
    })
}
initProfile();
...
$(".map-btn").click(function () {
    ...
    $(this).addClass("btn-primary");
    var mapName = $("input[name='selectMapOptions']:checked").attr('id');
