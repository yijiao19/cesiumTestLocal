...
var airView = initViewer('#cesiumContainer', "http://localhost:4002", "http://10.192.25.154:4002/?",
    {
...
function setTimeBar() {
    ...
    $("#verContentLable").html(conf.time.utc().format("YYYY-MM-DD HH:mm:ss") + " UTC " + conf.overlayType);
}
...
//绑定图层的change事件，当选择不同的地图时调用 SelectChange()方法
$("#selectLayer").change(function () {
    airView.changeLayer($('#selectLayer option:selected').val());
});
//选择预报还是实况
var monitorMode = false;
d3.select("#showToolBar").style("background","#48b").style("border-color","#aef");
$("#showToolBar").click(function () {
    if (monitorMode) {
        airView.disableMonitor();
        $("#selectMap").val("streetMap");
        $("#selectMap").show();
        $("#selectLayer").hide();
        d3.select("#showToolBar").style("background","#48b").style("border-color","#aef");
        d3.select("#showMonitor").style("background","#303336").style("border-color","#444");
        monitorMode = false;
    }
...
$("#showMonitor").click(function () {
    if (!monitorMode) {
        $("#toolbar").hide();
        airView.enableMonitor();
        hideVertical();
        $("#selectMap").hide();
        $("#selectLayer").show();
        d3.select("#showMonitor").style("background","#48b").style("border-color","#aef");
        d3.select("#showToolBar").style("background","#303336").style("border-color","#444");
        monitorMode = true;
    }
});
